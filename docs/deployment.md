...
- WAL 模式下趁运行只拷 `feedback.db` 单文件会得到不一致快照；如必须在线备份，用
  `sqlite3 data/feedback.db ".backup '<目标>'"` 或等价 WAL 安全备份（运行中逐个复制主库及 WAL/SHM 不能保证一致性）。
...
2. **新格式恢复数据**（`archive_data_json` V2：保留 V1 字段并增加逐份日志的恢复状态，V1 仍可读）随正文一起落库，备份即包含；`upload.credentialsEnc` 用主密钥 AES-256-GCM 加密，恢复主密钥备份才能解密预签名地址。
...
- Kaneo 写入结果不确定（超时/断连/5xx/服务在发送中崩溃）→ `needs_review`。管理页按 `recovery.allowedActions` 只渲染可用动作，全部动作携带 `expectedRevision` 乐观锁（过期页面 → `409 revision_conflict`，请刷新）：
  ...
  - **`retry_comment`（针对评论，唯一允许重发评论的动作）**：执行前按「反馈 ID + 附件摘要 + 当前资产引用」（日志再加日志 ID）查重；命中→补记确认并按完成条件收尾（零远端写入），未命中→发送**至多一条**评论。可选 `logId` 只针对某份日志。**仍可能产生重复评论**（远端列表与写入之间存在竞态），管理页在确认框中提示该风险，操作后请到 Kaneo 复核评论数量。
  - **`replace_upload`（针对附件，唯一可以替换附件的动作）**：先经鉴权下载核对真实字节（与本地截图/日志 SHA-256 比对；一致则仅补记，不替换）；替换时**复用原任务、保留旧 key 恢复记录（`replacedKeys`）、不自动删除远端对象**；替换成功后重发评论。可选 `logId` 只替换某份日志。核对失败或多候选 → `needs_review` 待人工核对，绝不猜测。
   - **只有全部附件完成才叫归档完成**：截图与每份日志各自独立上传、登记资产并挂评论；服务端在同一个集中检查里确认**所有**附件都已挂载才置 `archived`。仅截图完成而日志未完成时记录维持 `needs_review`（`error_summary` 会说明尚有附件未完成），管理页也**不会**显示“全部归档完成”。
...
- 日志随反馈一起存在 SQLite 的 `feedback_logs` 表（**内容以 BLOB 保存**），因此**数据库体积会随日志增长**：
  单份反馈最多 3 个日志、每个 ≤ 1 MiB，即单条反馈最多约 3 MiB 附件字节（截图另有 5 MiB 上限）。
  备份/迁移策略不变（备份 `FEEDBACK_DATA_DIR` 整个目录），但要按此估算磁盘占用。
- 数据库 schema 版本随本轮升到 **v3**（新增 `feedback_logs` 子表）。升级只需替换镜像并重启：
  服务启动时**自动增量迁移**（事务内建表，成功后写版本号，失败回滚且版本不变），
  **不重建 `feedbacks`、不改写历史记录**；带旧截图与恢复记录的库可直接升级，可重复启动。
- 反向代理：日志提交走同一个 `POST /api/feedback`，请求体上限由 nginx 的 `client_max_body_size 10m`
  兜住（服务端自身限制 9 MiB，含截图 5 MiB + 3 份日志各 1 MiB 与表单开销）。
- 管理页读取/下载日志走 `GET /api/admin/feedback/:id/logs/:logId`（同源 cookie 鉴权，
  `no-store` + `nosniff`，可加 `?download=1` 强制下载）；服务端**不产生公开链接**，
  也**不把附件内容写进运行日志**，排查问题时请用管理页预览或下载。

## 5. 反向代理（HTTPS）

反馈服务与 Kaneo 同域部署亦可。要求：透传 `Host`（同源检查依赖最终 URL）与 WebSocket 无关（本服务无 WS）。`X-Forwarded-Proto` 非必需；Origin 校验优先使用 `FEEDBACK_PUBLIC_URL` 的 origin，未配置时才使用请求 URL。Cookie `SameSite=Lax`，管理页与登录窗口均为第一方上下文，不依赖第三方 Cookie。

## 6. 已知边界（第一版）

- 单实例、进程内队列：水平扩展不支持（多实例会并发处理同一恢复队列）。
- 限流为进程内滑动窗口（登录 10 次/15 分钟/IP+用户名；提交 60 次/小时/会话）。
- 附件（截图与日志）归档依赖 Kaneo 上传接口与资产鉴权下载；远端对象在替换后不自动删除（须人工清理）。日志的自动采集由宿主通过 `logProvider` 提供；服务端不做系统日志扫描、不接收压缩包。
