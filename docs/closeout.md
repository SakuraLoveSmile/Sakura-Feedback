...
| T2 独立 Web／Android 示例可用 | 待体验 | Mi 10 真机全流程通过：拖球截图、遮挡、自动日志、文件选择添加/取消、预览移除、登录提交、切页、草稿重开；反馈 `10c4f8f4` 可在管理页下载两份日志。远端归档仍随 T1 的 S3 缺口 |
...
**7）部署方声明“已部署好”后的复验（同日更晚）：仍未恢复。** 同一把 Key、同一路径再发一次 PUT
仍是 **503** 且正文逐字相同；同时 `curl /api/openapi` 抓回 185473 B 与上午快照
`cmp` **逐字节相同**。即：该实例的代码与运行配置都没有变化，`S3_ENDPOINT`/`S3_BUCKET`
仍未进入 API 进程的环境。按“同一阻塞两次不同依据未解决即停手”的规则，本条不再重复尝试，
转入外部修复指引（第 5 节）。
...
### Android 真机实测结果（2026-09-12，T2 主体验收）

用户正常解锁并在开发者选项中开启「USB 调试（安全设置）」后，`adb shell input` 不再返回
`INJECT_EVENTS` 权限错误（此前为 `SecurityException: Injecting input events requires ...`，
确认属于系统确实拒绝自动输入，而非应用问题）。设备：Mi 10 / Android 16，ADB `192.168.0.19:40029`；
复用已安装示例 `dev.example.feedback_example`（versionName 1.0.0，依赖 feedback_widget 0.2.0），
其内嵌配置为 `FEEDBACK_API_BASE=http://192.168.0.23:8798`、`FEEDBACK_APP_ID=com.feedback.real-e2e-20260912`。
候选源码 `a29c0dc` 只改 e2e 脚本，未触及示例代码，故按计划**未重新构建 APK**。

逐项结果（截图证据在 `/tmp/mi10-*.png`）：

| 项 | 结果 |
|---|---|
| 拖球截图 | 拖动灵感球到界面中部松手即触发截图并打开面板；第一、第二路由各一次 |
| 遮挡 | 下载回传的截图（786×1702）中，「卡号 6222 0000 1234 5678」整块被**中性灰矩形遮罩**覆盖，其余为真实页面内容 |
| 自动日志 | 面板自动挂载 `host-app.log`（200 B，来源「自动」）；宿主缓冲计数随「写入一条运行日志」从 1→2→3 变化 |
| 文件选择·取消 | 打开系统文件选择器后 `BACK` 取消，返回面板，描述与日志列表**未受影响** |
| 文件选择·添加 | 经 `adb push` 放入 `/sdcard/Download/manual-android.txt`（137 B）后在「下载内容」中选中，面板出现 `manual-android.txt 137 B 手动` |
| 预览移除 | 点该条目的删除图标后列表退回只剩 `host-app.log`，其余状态不变 |
| 登录提交 | 用户名/密码登录成功（令牌存本机），提交后界面显示「处理中」→「失败（已记录，可在管理页处理）」，并提示"请勿重复提交、管理员可按 ID 恢复" |
| 切页 | 进入「第二个路由」后灵感球仍在场，该路由另有敏感区「邮箱 demo@example.com」 |
| 草稿重开 | 关闭面板并跨路由切换后重新打开，描述原文与截图草稿均完整恢复 |

服务端落库（`http://192.168.0.23:8798`）：

- 反馈 ID `10c4f8f4-df2a-4db3-8c90-7fb1e0ba7870`，状态 `needs_review`，`archiveStage=asset_uploading`，
  错误「Kaneo 附件或评论写入结果不确定，待核对」——**符合 Kaneo S3 未恢复时的预期**，不是本地缺陷。
- 描述与设备输入逐字一致：`Android real-device check - orb drag capture, sensitive card masked, auto log attached.`（87 B）。
- 截图 `786×1702`，91806 B，SHA-256 `060b9a5eb2a85335793eb9534ce4b813182e6a8e0e3538e4f54d470740ed54d6`。
- 日志两份，管理页下载字节与摘要：
  `host-app.log`（auto，200 B，`b59b2526895f3d650e48ce09342f8617873303989d9cd62ef02954968fc9c762`）、
  `manual-android.txt`（manual，137 B，`92f00ff102b0cc46cdbfa595204a07f59ba2502c433ffa99d0759f28e2855aed`）。
  其中 `manual-android.txt` 的摘要与**推送到手机前的源文件**完全相同，证明设备→服务链路逐字节无损。
- 提交后复查管理页列表仍为 3 条，**未产生重复反馈**。

测试环境经验（属测试装置，不是产品缺陷）：设备上的微信输入法/Sogou 会把 `adb shell input text`
的内容当作拼音组词，导致字符被重排或插入空格，密码框因此登录失败。解法是**完全绕开输入法**——
临时 `ime disable` 两个输入法后按字符注入 `input keyevent`（大写用 `input keycombination KEYCODE_SHIFT_LEFT`），
实测用户名与密码均可精确写入。测试后已恢复原默认输入法（微信输入法）。

### 剩余执行与状态规则

Docker 存储缺口已关闭：改为按 digest 拉取候选镜像（不重复本地构建），
`e2e/run_backup_restore.py` 已能直接吃 digest 并在干净候选上 34/34 通过。
T2 的设备阻塞已解除，真机流程全部通过。**剩余唯一缺口是 Kaneo 部署的 S3 配置**：
用户反馈"Kaneo 已部署好"后复验，线上预签名接口仍返回同一个 503，且
`/api/openapi` 与上午快照**逐字节相同**（`cmp` 无差异），说明该实例的运行环境没有变化。
需部署方设置后恢复 `b2d73aa3` 与 `10c4f8f4` 并做图文验收。
该缺口不是本仓库源码缺陷，本轮不再重复试错同一阻塞。

正式 v0.2.0 发布保留门禁：唯一剩余缺口（Kaneo S3）关闭后，从最终干净提交发布，
