...
个人使用的反馈收集组件 + 归档服务。流程：**呼出面板 →（可选）截取当前应用视口并自动遮挡敏感区 →（可选）自动附带/手动补充日志 → 输入文字 → 服务保存原话、截图与日志 → AI 结合日志整理 → 写入指定 Kaneo 项目的待筛选列**。

反馈是**截图 + 描述 + 日志**的多模态提交，不是只有文字：灵感球入口（`launcher-mode="orb"`）可以拖到界面任意位置指出问题，组件随即截取**当前应用视口**，在客户端完成像素级遮挡后，把截图与文字一起提交（`POST /api/feedback` 的 multipart `screenshot` 部件）。也可以只用文字——截图的默认值就是**关闭自动截图**（Web `capture-mode="off"`，Flutter `FeedbackCaptureMode.off`）；这种情况下面板里仍有手动的「截取当前页面」，用户想带图时随时可拍（先输入文字也能拍、失败可重试、移除后可补拍），截图与文字互不影响。
...
| `docs/logs-plan.md` | 本轮「截图＋描述＋日志」的实施契约（线格式、限制、存储、行为约定） |
| `docs/logs-delivery.md` | 本轮日志能力的交付记录与实测命令/结果 |

## 快速开始（开发）

当前收尾版本：**v0.2.0**（截图＋描述＋日志）。安装产物和真实服务、Android 验证状态见
[`docs/closeout.md`](docs/closeout.md)。真实服务启动使用 `scripts/start-service.sh`，
配置步骤见[接入入口](docs/integration.md#v020-接入入口截图描述与日志)。
`scripts/start-local.sh` 是 mock 演示，不能证明 AI／Kaneo 真实归档。
...
- 提交内容：文字上限 10,000 码点 + 可选截图（限制见上面"截图范围"）+ 可选日志（最多 3 份、每份 ≤ 1 MiB、仅 `.log`/`.txt`/`.json`/`.jsonl` 且须为合法 UTF-8）；页面上下文（`app-version` / `page-label`）由宿主显式传入，组件不自动抓取页面或参数。
- 日志只来自宿主已有日志系统：组件不拦截全局 console、不扫描磁盘、不自行收集网络请求；宿主通过 `logProvider` 显式导出，并负责在交给组件前脱敏。管理页可预览与下载日志；AI 仅在诊断中区分「日志证据」与「推测」，不把错误日志当成已证实根因。
- 原话先入库再返回“已接收”；后台处理可在服务重启后恢复（崩溃于 Kaneo 发送中的记录进入“待核对”，通过任务描述中的反馈 ID 核对，绝不盲目重复创建）。
- AI 只产出标题、结构化整理（使用体验/问题/建议/待确认）与可选诊断（日志证据 / 可能原因 / 推测），来源信息、原话引用与反馈 ID 由服务追加，AI 不决定项目、列或调度。
- Kaneo 归档按「先截图，再按顺序的日志」逐个上传与挂评论，每份附件独立记录；**只有全部附件都确认挂载才标记归档完成**，已确认附件不重复上传。
